...
export class AgendaService {
    ...
    constructor(@InjectModel(Show.name) private readonly agendaModel: Model<Show>) {}
    ...
    async create(createAgendaDTO: CreateShowDTO): Promise<Show> {
      ...
      async update(updateShowDTO: UpdateShowDTO): Promise<Show> {
        await this.agendaModel.findByIdAndUpdate(updateShowDTO._id, updateShowDTO).exec();
